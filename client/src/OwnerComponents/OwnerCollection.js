...
function OwnerCollection({ searchTerm, handleEditOwner, handleOwnerDelete, equipmentOwnerArray }) {
    ...
    const ownerCards = equipmentOwnerArray?.map((item) => {
            ...
            return <OwnerCard key={item.email} id={item.id} email={item.email} name={item.name} location={item.location} phone={item.phone} equipmentArray={item.equipment} profileImage={item.profileImage} handleEditOwner={handleEditOwner} item={item} handleOwnerDelete={handleOwnerDelete}/>}
    })
